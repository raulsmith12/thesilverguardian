...
function validatePetitionSignature(body) {
  ...
  else if (data.postalCode.length > 20) errors.postalCode = "Must be 20 characters or fewer";
...
async function createPetitionSignature(input) {
  const [result] = await getDatabasePool().execute(
    `INSERT INTO petition_signatures (name, email, postal_code) VALUES (?, ?, ?)`,
    [input.name, input.email, input.postalCode],
  );
