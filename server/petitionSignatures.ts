...
export type PetitionSignatureInput = { name: string; email: string; favoriteHockeyTeam: string; consent: true };
...
export function validatePetitionSignature(body: unknown): ValidationResult {
  ...
  const data = { name: normalizeText(source.name), email: normalizeText(source.email).toLowerCase(), favoriteHockeyTeam: normalizeText(source.favoriteHockeyTeam), consent: source.consent === true };
  ...
  if (!data.favoriteHockeyTeam) errors.favoriteHockeyTeam = "Required"; else if (data.favoriteHockeyTeam.length > 255) errors.favoriteHockeyTeam = "Must be 255 characters or fewer";
...
export async function createPetitionSignature(input: PetitionSignatureInput) {
  const [result] = await getDatabasePool().execute<ResultSetHeader>(`INSERT INTO petition_signatures (name, email, favorite_hockey_team) VALUES (?, ?, ?)`, [input.name, input.email, input.favoriteHockeyTeam]);
