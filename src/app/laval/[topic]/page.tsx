...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const content = getLavalTopicContent(topic, "en");
  if (!content) return {};
  return createPageMetadata({ title: content.title, description: content.description, path: `/laval/${topic}/`, keywords: content.keywords });
}

export default async function LavalTopicPage({ params }: Props) {
  const { topic } = await params;
  const content = getLavalTopicContent(topic, "en");
  if (!content) notFound();
