...
const details: Record<WashingtonAreaSlug, AreaDetail> = {
  ...
  renton: {
    ...
    descriptionEn: "Explore Renton resources for child-friendly environments, adaptable movement and responsible sports-inspired community support.",
    descriptionFr: "Découvrez les ressources de Renton sur les milieux adaptés aux enfants, le mouvement et l’appui inspiré du sport.",
  ...
  bellevue: {
    ...
    descriptionEn: "Explore Bellevue resources for pediatric environments, accessible movement and careful sports-inspired community concepts.",
    descriptionFr: "Découvrez les ressources de Bellevue sur les milieux pédiatriques, le mouvement accessible et les concepts inspirés du sport.",
  ...
  everett: {
    ...
    descriptionEn: "Explore Everett resources for pediatric environments, accessible movement and responsible sports-inspired community concepts.",
    descriptionFr: "Découvrez les ressources d’Everett sur les milieux pédiatriques, le mouvement accessible et les concepts inspirés du sport.",
...
const areaLinks = (locale: Locale, current: WashingtonAreaSlug): MontrealPageContent["relatedLinks"] => {
  ...
  return [
    { href: `/${current}/kid-friendly-research-hospitals/`, label: isFrench ? `hôpitaux de recherche adaptés aux enfants à ${details[current].name}` : `kid-friendly research hospitals in ${details[current].name}`, description: isFrench ? "Découvrez la vision locale d’un milieu pédiatrique inspiré du sport et dirigé par un fournisseur volontaire." : "Explore the local vision for a provider-led, sports-inspired pediatric environment." },
...
export function getWashingtonAreaContent(slug: string, locale: Locale): MontrealPageContent | undefined {
  ...
  return {
    ...
    supportingImageAssets: [
      { src: pediatricEnvironmentHero, alt: isFrench ? "Salon familial pédiatrique accessible et subtilement inspiré du sport" : "Accessible pediatric family lounge with subtle sports-inspired details" },
    ...
    sections: [
      ...localSections,
      {
        ...
        paragraphs: isFrench
          ? ["La vision d’un hôpital de recherche adapté aux enfants concerne un milieu non clinique inspiré du sport qui ne pourrait être envisagé qu’avec un fournisseur volontaire dirigeant les décisions, la sécurité et la vie privée. Aucun hôpital, chantier, recherche, programme ou partenariat local n’est annoncé.", "Le concept des Centres de thérapie par le mouvement concerne des activités communautaires artistiques, adaptées et inspirées du sport. Il ne constitue ni traitement, ni physiothérapie, ni réadaptation. Aucun lieu, cours, instructeur, horaire ou processus d’inscription local n’est confirmé."]
          : ["The kid-friendly research hospital vision concerns a non-clinical, sports-inspired environment that could only be considered if a willing healthcare provider led decisions, safety and privacy. No local hospital, construction, research, program or partnership is announced.", "The Movement Therapy Centers concept concerns artistic, adapted and sport-inspired community activity. It is not treatment, physical therapy or rehabilitation. No local venue, class, instructor, schedule or registration process is confirmed."],
...
function areaSeriesLinks(locale: Locale, areaSlug: WashingtonAreaSlug): MontrealPageContent["relatedLinks"] {
  ...
  return [
    ...
    { href: `/${areaSlug}/kid-friendly-research-hospitals/`, label: isFrench ? `hôpitaux de recherche adaptés aux enfants à ${name}` : `kid-friendly research hospitals in ${name}`, description: isFrench ? "Explorez une vision de milieux pédiatriques inspirés du sport et dirigés par des fournisseurs volontaires." : "Explore a vision for provider-led pediatric environments inspired by sports." },
    { href: `/${areaSlug}/movement-therapy-centers/`, label: isFrench ? `centres de thérapie par le mouvement à ${name}` : `Movement Therapy Centers in ${name}`, description: isFrench ? "Découvrez un concept accessible de mouvement artistique, adapté et inspiré du sport." : "Explore an accessible concept for artistic, adapted and sport-inspired movement." },
  ];
}
    ...
    isHospital
      ...
      : {
          ...
          paragraphs: [
            ...
            `Dance, rhythm, balance, adapted games or non-contact sport-inspired drills could offer seated, standing, mobility-device, observation-based and low-sensory ways to participate. They would remain community activities, not treatment, physical therapy or rehabilitation. No local class, instructor, fee, schedule or registration process is announced, and no one should change medical care or exercise based on this page.`,
          ],
        },
  ];
}
...
export function getWashingtonAreaTopicContent(
  ...
  return {
    ...
    description: isHospital
      ? isFrench
        ? `Découvrez comment un milieu pédiatrique inspiré du sport pourrait être envisagé à ${detail.name} sous la direction d’un fournisseur volontaire.`
        : `Explore how a provider-led, sports-inspired kid-friendly pediatric environment could be considered in ${detail.name} with clear boundaries.`
      : isFrench
    ...
    keywords: isHospital
      ? isFrench
        ? [`hôpitaux de recherche adaptés aux enfants ${detail.name}`, `milieu pédiatrique ${detail.name}`, `hôpital sportif ${detail.name}`]
        : [`kid-friendly research hospitals ${detail.name}`, `children's research hospital ${detail.name}`, `sports-themed pediatric environment ${detail.name}`]
      : isFrench
    ...
    imageAlt: isFrench
      ? isHospital
        ? `Milieu pédiatrique accueillant inspiré du sport à ${detail.name}`
        : `Activité communautaire de mouvement accessible à ${detail.name}`
      : isHospital
        ? `Welcoming sports-inspired pediatric environment in ${detail.name}`
